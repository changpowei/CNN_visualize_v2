...
class BaseCAM(BaseModel):
    ...
    def get_classmap(self, label, conv_out, input_im):
        """
        Compute class activation map(CAM) of class = label with name 'classmap'
        ...
        Args:
            ...
            conv_out (tf.tensor): 4-D Tensor of shape
                ...
                convolutional layers. For now is conv_cam.
            input_im (tf.tensor): A 4-D Tensor image.
                The original model input image patch.
        """
        # Get original image size used for interpolation
        o_height = tf.shape(input_im)[1]        #原圖的長
        o_width = tf.shape(input_im)[2]         #原圖的寬

        # Get shape of output of convolution layers
        conv_out_channel = tf.shape(conv_out)[-1]   #conv_cam(feature maps) have 1024 channels
        conv_height = tf.shape(conv_out)[1]         #最後一層捲基層輸出(即特徵圖)的長
        conv_width = tf.shape(conv_out)[2]          #最後一層捲基層輸出(即特徵圖)的寬

        # Get weights corresponding to class = label
        with tf.variable_scope('cam') as scope:
            scope.reuse_variables()
            
            #label_w: fc_w(1024, 257)對轉置得(257, 1024)，取第label(如：caltech-256中161是狗)的row得(1, 1024)
            label_w = tf.gather(
                tf.transpose(tf.get_variable('weights')), label)
            #reshape to (1024, 1)
            label_w = tf.reshape(label_w, [-1, conv_out_channel, 1])
            #tile: 代表在不同維度複製，測試data為1個batch，故在3個維度複製1次，等於根本沒有動作，仍為1024*1
            label_w = tf.tile(label_w, [tf.shape(conv_out)[0], 1, 1])
        
        #將feature map reshape為(heigh*width, 1024)
        conv_reshape = tf.reshape(
            conv_out, [-1, conv_height * conv_width, conv_out_channel])
        #將每張特徵圖乘上該特徵圖的權重 => (heigh*width, 1024) * (1024, 1)
