...
import sys
sys.path.append('../')
from lib.models.cam import VGGCAM
...
def get_config(FLAGS):
    ...
    training_callbacks = [
        ModelSaver(periodic=100),                                           #每100個training step就儲存model到config.checkpoint_dir裡面。
        TrainSummary(key='train', periodic=50),                             #每50個training step就會儲存訓練過程的準確率、loss等資訊到summary，方便之後用tensorboard查看
        FeedInferenceBatch(dataset_val, batch_count=10, periodic=100,       #每100個training step就透過所有dataset_val(同dataset_train)去測試準確率與lost。
                           inferencers=inference_list_validation),
        CheckScalar(['accuracy/result', 'loss/result'], periodic=10)]       #每10個training step就顯示準確率與lost。
    ...
    if FLAGS.label > 0:
        ...
        training_callbacks += FeedInference(dataset_test, periodic=50,      #每50個training step就測試infer image，並將測試結果儲存到config.infer_dir裡面。
                                            infer_batch_size=1,
...
def get_predict_config(FLAGS):
    ...
    return PridectConfig(
        ...
        #測試時指定inspect_class(FLAGS.label)會儲存某一類別的CAM。
...
def get_args():
    ...
    parser.add_argument('--label', default=161, type=int,
                        help='Label of inspect class.')
    parser.add_argument('--nclass', default=257, type=int,
                        help='number of image class. Like Caltech-256 dataset is 257.')
    ...
    parser.add_argument('--type', default='.jpg', type=str,
                        help='image type for training and testing. Default = .jpg')
    
    parser.add_argument('--infer_type', default='.png', type=str,
                        help='type of inference image using for training. Default = .png')

    parser.add_argument('--model', default='model-24800', type=str,
                        help='file name of the trained model. Like "model-24800"')
